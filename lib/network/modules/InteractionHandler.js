...
class InteractionHandler {
  ...
  onMouseMove(event) {
    ...
    if (this.popup !== undefined) {
      ...
      if (this.popup.hidden === false && this.options.tooltipSticky === false) {
        let pointerX = this.options.tooltipSticky ? pointer.x + 3 : pointer.x;
        let pointerY = this.options.tooltipSticky ? pointer.y - 5 : pointer.y;

        popupVisible = true;

        this.popup.setPosition(pointerX, pointerY);
        this.popup.show();
      }
    }
 ...
 _checkShowPopup(pointer) {
    ...
    if (this.popupObj !== undefined) {
      ...
      if (this.popupObj.id !== previousPopupObjId) {
        if (this.popup === undefined) {
          this.popup = new Popup(this.canvas, '', this.options.tooltipSticky);
        }
 ...
 _checkHidePopup(pointer) {
    ...
    if (this.popup.popupTargetType === 'node') {
      if (this.body.nodes[this.popup.popupTargetId] !== undefined) {
        ...
        if (this.options.tooltipSticky && stillOnPopup === false && stillOnObj === false) {
          stillOnPopup = this.popup.isOverlappingWith(pointerObj);
          // console.log({ isOverlappingWith: stillOnPopup})
        }
      }
    }
    else {
    ...
    if (stillOnObj === false && stillOnPopup === false) {
      this.popupObj = undefined;
      this.popup.hide();
      this.body.emitter.emit('hidePopup');
    }
