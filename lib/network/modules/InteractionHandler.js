...
class InteractionHandler {
  ...
  setOptions(options) {
    if (options !== undefined) {
      this._checkRemoveZoomMinMax(options);
  ...
  /**
   * Build pointer object with both canvas and DOM values
   * @param {{x: number, y: number}} pointer  Object with the x and y screen coordinates of the mouse
   * @returns {{}}
   */
  getEnhancedPointer(pointer) {
    let enhancedPointerObjectCanvas = this.selectionHandler._pointerToPositionObject({
      x: pointer.x,
      y: pointer.y
    });
    ...
    return {
      canvas: {
        ...enhancedPointerObjectCanvas
      },
      DOM: {
        left: this.canvas._XconvertCanvasToDOM(enhancedPointerObjectCanvas.left),
        top: this.canvas._YconvertCanvasToDOM(enhancedPointerObjectCanvas.top),
        right: this.canvas._XconvertCanvasToDOM(enhancedPointerObjectCanvas.right),
        bottom: this.canvas._YconvertCanvasToDOM(enhancedPointerObjectCanvas.bottom)
      }
    };
  }
  ...
  _showPopup(pointer, fireEvent = false) {
    let enhancedPointer = this.getEnhancedPointer(pointer);

    // don't continually move popup when sticky
    if (this.options.tooltipSticky === false) {
      this.popup.setPosition(pointer.x + 3, pointer.y - 5);
    }

    this.popup.setText(this.popupObj.getTitle());
    this.popup.show(enhancedPointer);
  ...
  _getActiveNodeId(pointer) {
    ...
    // if there are overlapping nodes, select the topmost one
    if (overlappingNodes.length > 0) {
      activeNodeId = nodes[overlappingNodes[overlappingNodes.length - 1]].id;
    }

    return activeNodeId;
  }

  _getOverlappingEdgeId(pointer) {
    let pointerObj = this.selectionHandler._pointerToPositionObject(pointer);
    ...
    // if there are overlapping edges, select the topmost one
 ...
 _checkShowPopup(pointer) {
    ...
    let popupType = 'node';
    let activeNodeId = this._getActiveNodeId(pointer);
    ...
    if (this.popupObj === undefined && activeNodeId === undefined) {
      const overlappingEdgeId = this._getOverlappingEdgeId(pointer);
    ...
    if (this.popupObj !== undefined) {
      ...
      if (this.popupObj.id !== previousPopupObjId) {
        if (this.popup === undefined) {
          this.popup = new Popup(this.canvas.frame, this.options.tooltipSticky);
        }

        this.popup.popupTargetType = popupType;
        this.popup.popupTargetId = this.popupObj.id;

        this._showPopup(pointer, true);
      }
    }
    else {
      this._hidePopup();
 ...
 _checkHidePopup(pointer) {
    let pointerObj = this.selectionHandler._pointerToPositionObject(pointer);
    let isPointerOverObject = false;
    let isPointerOverPopup = false;
    let popupTargetId = this.popup.popupTargetId;

    if (this.popup.popupTargetType === 'node') {
      let nodes = this.body.nodes;

      if (nodes[popupTargetId] !== undefined) {
        isPointerOverObject = nodes[popupTargetId].isOverlappingWith(pointerObj);

        // if the mouse is still one the node, we have to check if it is not also on one that is drawn on top of it.
        // we initially only check isPointerOverObject because this is much faster.
        if (isPointerOverObject === true) {
          let overNode = this.selectionHandler.getNodeAt(pointer);
          isPointerOverObject = overNode === undefined ? false : overNode.id === popupTargetId;
        }

        // check to see if mouse pointer is within the popup
        if (this.options.tooltipSticky && isPointerOverPopup === false) {
          let enhancedPointer = this.getEnhancedPointer(pointer);
          isPointerOverPopup = this.popup.isOverlappingWith(enhancedPointer);
        }
      }
    }
    else {
   ...
   * @private
   */
  _checkRemoveZoomMinMax(options) {
    const { zoomMin, zoomMax } = options;

    const isValid = value => (
      util.isNumber(value)
        && value >= this.defaultOptions.zoomMin
        && value <= this.defaultOptions.zoomMax
    );
