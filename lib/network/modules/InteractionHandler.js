...
class InteractionHandler {
  ...
  _shouldHidePopup(pointer) {
    let isPointerOverObj = false;

    if (this.popup.popupTargetType === 'node') {
      isPointerOverObj = this._isPointerOverNode(pointer);
    }
    else {
      isPointerOverObj = this._isPointerOverEdge(pointer);
    }

    return !isPointerOverObj;
  }
   ...
   * @private
   */
  _isPointerOverNode(pointer) {
    let pointerObj = this.selectionHandler._pointerToPositionObject(pointer);
    let isPointerOverNode = false;

    if (this.body.nodes[this.popup.popupTargetId] !== undefined) {
      isPointerOverNode = this.body.nodes[this.popup.popupTargetId].isOverlappingWith(pointerObj);

      // if the mouse is still one the node, we have to check if it is not also on one that is drawn on top of it.
      // we initially only check isPointerOverNode because this is much faster.
      if (isPointerOverNode === true) {
        let overNode = this.selectionHandler.getNodeAt(pointer);
        isPointerOverNode = overNode === undefined ? false : overNode.id === this.popup.popupTargetId;
      }
    }

    return isPointerOverNode;
  }

  /**
   * Check if pointer is over an edge
   * @param {{x:number, y:number}} pointer
   * @returns {boolean}
   * @private
   */
  _isPointerOverEdge(pointer) {
    let pointerObj = this.selectionHandler._pointerToPositionObject(pointer);
    // let isPointerOverEdge = false;

    if (this.selectionHandler.getNodeAt(pointer) === undefined) {
      if (this.body.edges[this.popup.popupTargetId] !== undefined) {
        return this.body.edges[this.popup.popupTargetId].isOverlappingWith(pointerObj);
      }
    }
