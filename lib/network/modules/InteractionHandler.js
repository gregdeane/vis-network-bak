...
class InteractionHandler {
   ...
   * @param {{}} enhancedPointer    // Pointer containing DOM/canvas x/y values
   * @returns {{}}
   */
  buildEnhancedPointer(enhancedPointer) {
    let enhancedPointerObjectCanvas = this.selectionHandler._pointerToPositionObject({
      x: enhancedPointer.DOM.x,
      y: enhancedPointer.DOM.y
    });
  ...
  _showPopup(pointer, fireEvent = false) {
    let properties = this.selectionHandler._initBaseEvent('showPopup', pointer);

    if (this.popup !== undefined) {
      let enhancedPointer = this.buildEnhancedPointer(properties.pointer);
